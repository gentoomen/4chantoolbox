...
main = withCurlDo $ do
    ...
    unless (quiet args) $ putStr $ "Page retrieved. " ++ show (length images) ++ " images found.\n"
    forM_ (concat images) $
        \a -> do 
            getImage a (output args)
            unless (quiet args) $ putStrLn $ "Downloaded " ++ show a ++ (if output args == "./" then "" else " to " ++ output args)